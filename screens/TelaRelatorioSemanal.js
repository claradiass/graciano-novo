  ...
  const generateDailyData = (date) => {
    const selectedData = servico.filter((item) => {
      return (
        ...
        item.attributes.dataFinalizado.startsWith(date)
      );
    });
    ...
    if (selectedData.length > 0) {
      ...
      selectedData.forEach((item) => {
        if (
          item.attributes &&
          typeof item.attributes.totalDespesas === "number" &&
          !isNaN(item.attributes.totalDespesas)
        ) {
        ...
        if (
          item.attributes &&
          typeof item.attributes.valorTotal === "number" &&
          !isNaN(item.attributes.valorTotal)
        ) {
