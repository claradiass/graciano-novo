...
export default function TelaManutencaoAtrasados({ route }) {
  ...
  useEffect(() => {
    axios
      .get(baseUrlServicos + "?populate=*", configAxios)
  ...
  function remover() {
    axios
      .delete(baseUrlServicos + data.id, configAxios)
      .then(function (response) {
  ...
  useEffect(() => {
    if (searchText === "") {
      ...
      setList(
        servicos.filter(
          (item) =>
            item.attributes.aparelho
              .toLowerCase()
              .indexOf(searchText.toLowerCase()) > -1 ||
            (item.attributes.outros &&
              item.attributes.outros
                .toLowerCase()
                .indexOf(searchText.toLowerCase()) > -1) ||
            (item.attributes.cliente.data?.attributes.nome &&
              item.attributes.cliente.data?.attributes.nome
                .toLowerCase()
                .indexOf(searchText.toLowerCase()) > -1) // // item.attributes.cliente.data.attributes.nome.toLowerCase().indexOf(searchText.toLowerCase()) > -1
        )
      );
    }
  }, [searchText]);
